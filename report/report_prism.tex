...
{\Large\bfseries Project report} \\ \vspace{10mm}
by \\ \vspace{2mm}
{\Large\bfseries Claire Pennarun \\ Tatiana Rocher }\\
Master 2 Algorithmes et Méthodes Formelles \\ \vspace{10mm}
{\large\bfseries Université de Bordeaux}\\ \vspace{1cm}
\centerline{\line(1,0){500}} \vskip 0.5cm
{\huge\bfseries Efficient, Proximity-Preserving Node Overlap Removal\\
~~ \\
\large Article by E.R. Gansner and Y. Hu (2009)}\\ \vspace{0,2cm}
\centerline{\line(1,0){500}} \vskip 1.5cm
Supervisors \\ \vspace{2mm}
{\large\bfseries M. Pinaud Bruno \\ M. Narbel Philippe}\\ \vspace{1cm}
January, $12^{th}$, 2014 \vspace{10mm}
...
\begin{figure}[h]
  ...
  \setlength\fboxsep{5pt}
  \setlength\fboxrule{0.5pt}
  \fbox{\includegraphics[width=0.7\textwidth]{figures/badcase.png}}
...
The authors mainly compared the results of the PRISM algorithm with the results of the Solve\_VPSC algorithm on 14 graphs provided by the GraphViz suite \cite{Gansner02}. They do note that PRISM gives generally smaller dissimilarities measures and that they are most of the time correlated.

Because of its implementation (it moves the nodes in the vertical and horizontal directions alternately), Solve\_VPSC often creates very tall and thin, or very thick and wide layouts. This is not always detected by the two measures proposed by the authors.
...
The algorithm stops when all considered nodes have reached $\Delta_i < \varepsilon$ or if no changes of positions have been made in the preceding iteration. As $\varepsilon \neq 0$, it happens that all nodes have not been removed.
...
We can see a lot of overlaps in the green part. The boxes $b'2$ and $b'3$ do overlap in the axis and in the graph. But the boxes $b'1$ and $b'2$ overlap in the axis and do not overlap in the graph. This is resolved by the line 5 of the algorithm by a check in the graph (we use the method \texttt{BoundingBox.contains(BoundingBox)}).
...
\begin{figure}[h]
  ...
  \fbox{\includegraphics[width=0.9\textwidth]{figures/b124b.png}}
  
  \fbox{\includegraphics[width=1\textwidth]{figures/b124c.png}}
...
\bigskip
As our implementation moves only one node at a time, some overlaps are often created during the first phase, but are generally removed at the next iteration.

We thus wanted to know how the number of overlaps found along the edges of the Delaunay triangulation evolved. We took the now well-known \texttt{b124} graph and displayed the number of overlaps found during the first stage of our algorithm at each iteration (see \ref{overlapsb124_1}). The number of overlaps found decreases quite quickly and then becomes steady.  
...
\begin{figure}[h]
  ...
  \setlength\fboxsep{5pt}
  \setlength\fboxrule{0.5pt}
  \fbox{\includegraphics[width=0.8\textwidth]{figures/overlapsb124.png}}
  ...
  \label{overlapsb124_1}
\end{figure}

The same convergence of the first phase can be seen if we change the value of $\varepsilon$ (see figure \ref{overlapsb124_2})

\begin{figure}[h]
	\center
  \setlength\fboxsep{5pt}
  \setlength\fboxrule{0.5pt}
  \fbox{\includegraphics[width=0.8\textwidth]{figures/overlapsb124_2.png}}
  
  \caption{The number of overlaps found during the first phase of the algorithm at each iteration - \texttt{b124} graph - $\varepsilon = 0.000001$}
  \label{overlapsb124_2}
\end{figure}

\bigskip
But we found also other graphs on which our algorithm is not so efficient. On the \texttt{b102} graph (see figure \ref{b102}), which has 302 nodes and 611 edges, the nodes placement is causing a very large number of overlaps. This means that in order to run the algorithm in a reasonable time, we have to increase the $\varepsilon$ value. This leads to a algorithm creating a layout which is not overlaps-free (see figure \ref{overlapsb102}) nor quickly generated.
This is a problem which we did not have the time to study in details, but the compromise between efficiency and speed can be (as for the \texttt{b102} graph) quite difficult to found.

\begin{figure}[h]
	\center
  \setlength\fboxsep{5pt}
  \setlength\fboxrule{0.5pt}
  \fbox{\includegraphics[width=0.7\textwidth]{figures/b102_init.png}}
  
  \caption{The \texttt{b102} graph}
  \label{b102}
\end{figure}
...
\begin{figure}[h]
  ...
  \setlength\fboxsep{5pt}
  \setlength\fboxrule{0.5pt}
  \fbox{\includegraphics[width=0.8\textwidth]{figures/overlapsb102.png}}
  
  \caption{The number of overlaps found during the first phase of the algorithm at each iteration - \texttt{b102} graph - $\varepsilon = 0.001$}
  \label{overlapsb102}
\end{figure}

\subsubsection{Possible improvements and future work}

Some steps and parameters of our algorithm could clearly be improved.

\bigskip
The resolution of the stress model could be done an other way, since our implementation is still quite low. It will help applying the algorithm on larger graphs and with a lower value of $\varepsilon$.

An other point is the termination of the \texttt{while} loops, it may be a interesting point to apply the changes in the nodes positions only if the number of overlaps do not increase because of these changes (or if the difference between the numbers is lower than a certain threshold to determine).

\bigskip
The scan-line algorithm used in the second loop can be improved too. Its implementation is quite efficient but still quite basic, and a "real" scan-line algorithm would be a good improvement.

\bigskip
The effect of the $\varepsilon$ value and of the initial positions of nodes could be studied in more details, as it clearly influences the result of our algorithm (and sometimes even causes its failure). It may be interesting to ``scale" the given layout before applying the algorithm, in order to avoid having too many overlaps.
