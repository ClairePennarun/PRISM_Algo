...
As we want to find the remaining overlaps, the scan-line algorithm has to consider all the points of the graph layout. But we found a implemented class in Tulip, \texttt{BoundingBox}, which helped us to make a fast scan-line algorithm. In Tulip, each bounding box is represented by two points : the lowest and leftest point and the highest and rightest point of the bounding box.
...
In the figure \ref{scanline} we show an exemple of our scan-line algorithm \ref{scanline}. We have colored in red the graph and in green the list of bounding boxes sorted by abscissa. 
...
Moreover, the boxes $b'1$ and $b'3$ do not overlap in the abscissa, so none of the boxes higher that $b3$ can overlap with $b1$.
