...
But there are some situations where keeping $l_{ij}$ is not a good idea \texcolor{red}{(see figure)} % faire un dessin d'apres l'article
...
% et on est dans un graphe rigide
...
After the first step, it is possible that some overlaps are still not remove. To resolve that, we have to use a scan-line algorithm.
% mettre VPSC dans la biblio
\subsubsection{Scan-line algorithm}

A scan-line algorithm is a algorithm which will considere all the points of a layout. \\
We can not use a algorithm which use the graph properties because the vertex don't know the position of the others vertex, so we can't find the overlap by using the vertex properties.\\
For all the ordinate's points we considere all the points in the abscissa. If there is an overlap in a point, we add the edge (between the two vertices overlapping) in the Delaunay triangulation. 
% à etoffer


\bigskip
It is notable that the opponent algorithm, VPSC, only use a scan-line algorithm \textcolor{red}{(mettre lien)}
